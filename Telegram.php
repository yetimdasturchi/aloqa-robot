...
class Telegram {
    ...
    public function send_photo($photo, $caption = '', $chat_id = null)
    {
        $chat_id = (!is_null($chat_id)) ? $chat_id : $this->chat_id;
        $data = compact('chat_id', 'photo', 'caption');
        if (((!file_exists($photo)) && (filter_var($photo, FILTER_VALIDATE_URL) === FALSE))) {
    ...
    public function send_video($video, $caption = '', $chat_id=null, $duration = null)
    {
        $chat_id = (!is_null($chat_id)) ? $chat_id : $this->chat_id;
